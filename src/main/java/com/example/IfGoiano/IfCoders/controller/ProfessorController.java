...
public class ProfessorController {
    ...
    public ResponseEntity<?> update(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    required = true,
                    content = @Content(schema = @Schema(implementation = ProfessorUpdateDTO.class))
            )
            @org.springframework.web.bind.annotation.RequestBody ProfessorUpdateDTO professor,
            @AuthenticationPrincipal UserDetails userDetails
    ) {
        if (userDetails == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Usuário não autenticado");
        }
        return new ResponseEntity<>(professorService.update(professor, userDetails.getUsername()), HttpStatus.NO_CONTENT);
    }
